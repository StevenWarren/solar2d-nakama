local client_async = require "plugin.nakama.websocket.client_async"

local log = require "plugin.nakama.util.log"
local b64 = require "plugin.nakama.util.b64"
local uri = require "plugin.nakama.util.uri"
local json = require "plugin.nakama.util.json"
local uuid = require "plugin.nakama.util.uuid"
...
function M.socket_create(config, on_message)
	...
	socket.timer_handle = timer.performWithDelay(100, function(self, handle, time_elapsed)
		socket.ws:step()
	end, -1)
